...
import type { StripeInvoice } from '../services/api';
        ...
        <div className="p-6">
          {!(isActivePaid || isCancelled) ? (
            ...
            <div className="p-4 bg-red-50 border border-red-100 rounded-xl text-sm text-red-700 flex items-center gap-2">
              <AlertCircle size={14} /> {invoicesError}
            </div>
          ) : invoices.length === 0 ? (
            <div className="p-8 bg-slate-50 rounded-2xl text-center space-y-3">
              <p className="text-sm text-slate-500">
                No invoices yet. Your next billing cycle's invoice will appear here.
              </p>
              <button
                className="px-5 py-2 bg-slate-100 text-slate-700 rounded-xl text-sm font-semibold hover:bg-slate-200 transition-colors disabled:opacity-50"
                onClick={handleManageSubscription}
                disabled={portalLoading}
              >
            ...
            <div className="space-y-3">
              <div className="overflow-hidden border border-slate-100 rounded-2xl">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {invoices.map((inv) => {
                      const dt = new Date(inv.created * 1000);
                      const amount = (inv.total / 100).toLocaleString(undefined, {
                        style: 'currency',
                      ...
                      const statusCls = (inv.status && statusStyles[inv.status]) || 'bg-slate-50 text-slate-500 border-slate-200';
                      return (
                        <tr key={inv.id} className="border-t border-slate-100 hover:bg-slate-50/60">
                          ...
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border ${statusCls}`}>
                              {inv.status || 'unknown'}
                            </span>
                          </td>
                            ...
                            <div className="inline-flex items-center gap-3">
                              {inv.hostedInvoiceUrl && (
                                <a href={inv.hostedInvoiceUrl} target="_blank" rel="noopener noreferrer"
                                   className="text-xs font-semibold text-blue-600 hover:text-blue-700">View</a>
                              )}
                              {inv.invoicePdf && (
              ...
              <div className="flex justify-end">
                <button
                  className="px-4 py-2 text-xs font-semibold text-slate-600 hover:text-slate-800 transition-colors inline-flex items-center gap-1.5 disabled:opacity-50"
                  onClick={handleManageSubscription}
                  disabled={portalLoading}
                >
                  {portalLoading ? 'Opening…' : <>Manage in Stripe <ArrowUpRight className="w-3 h-3" /></>}
                </button>
              </div>
            </div>
          )}
