 ...
 * the documented dev/test behavior. With SIGCORE_WEBHOOK_HEALTH_OWNER set
 * to a host other than this instance's resolved expected URL host, check #7
 * also skips (owner gate).
 */
...
(async () => {
  ...
  console.log('  SIGCORE_WEBHOOK_HEALTH_OWNER:', process.env.SIGCORE_WEBHOOK_HEALTH_OWNER ?? '(unset)');
  ...
  // Hourly cron. The resolver mutates rows older than STALE_PENDING_HOURS (=6),
  // which by definition are stuck and should be marked. Running it manually
  // advances the same state the cron would have produced at :15 of the next hour.
