...
export class ThumbtackAdapter implements IPlatformAdapter {
  ...
  getAuthUrl(_userId: string, state: string, forceLogin = false, callbackUrl?: string): string {
    const params = new URLSearchParams({
      ...
      // associate-phone-numbers.read/.write added 2026-06-12 — required by
      // the per-business associate-phone sync (owner phone + LB number +
      // additionals). Without these, listAssociatePhones / registerAssociatePhone
      // 401 with "required scope(s) [supply::businesses/associate-phone-numbers.write]
      // not found". Existing tenants must re-OAuth to receive the new grants.
