...
export class UsersService {
  ...
  async updateProfile(userId: string, updates: {
    ...
    const user = await this.prisma.user.update({
      ...
      select: {
        ...
        additionalAssociatePhonesJson: true,
      },
      data,
    });
  ...
  async seedBusinessInfoFromAccount(
    ...
    /** 5-8 sentence prose summary GPT-4o-mini produced from the scrape.
     *  Already generated by verifyWebsite → summarizeWebsite; was being
     *  thrown away by this code path. Same surface the WebsitePreviewCard
     *  uses for the website branch. Absent when the Partner-API fallback
     *  ran (no HTML to summarize). */
    summary?: string;
    ...
    if (source === 'thumbtack') {
      ...
      if (ttProfileUrl) {
        try {
          ...
          // Capture the prose summary independently of playbookSeed —
          // even if structured extraction misses fields, the summary is
          // useful on its own and was being silently discarded.
          if (verify.reachable && verify.metadata?.summary) {
            summary = verify.metadata.summary;
          }
        } catch (err: any) {
  ...
  async seedBusinessInfoFromText(
    ...
    extractedFields?: Record<string, string | string[]>;
    /** GPT prose summary of the pasted text, same surface as the URL path. */
    summary?: string;
    conflictsRaised: number;
    warning?: string;
  }> {
  ...
  async applyBusinessProfileUrl(
    ...
    extractedFields?: Record<string, string | string[]>;
    /** GPT-generated prose summary of the scraped page. Populated for all
     *  three branches (website, TT, Yelp) when the scrape produced HTML
     *  that summarizeWebsite could process. Same content as
     *  websiteMetadata.summary on the website branch — duplicated here so
     *  the TT/Yelp paths get a single, predictable place to read it
     *  without having to reach into websiteMetadata (which is undefined
     *  for those branches). */
    summary?: string;
    conflictsRaised: number;
